...
The biggest product risk is trying to do everything and finishing nothing. Explicitly listing what to skip is **scope control**.
...
PDF File
  -> [extract text]          # PyPDF2 reads text from the PDF
  -> pages[]
  -> [build prompt: pages + question]  # Combine extracted text with user's question into an LLM prompt
  -> [LLM]
  -> Answer with [Page X]
